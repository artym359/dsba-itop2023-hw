#include "table4kmbyowner.h"
#include <QFile>
#include <QTextStream>
#include <QSize>

table4kmbyowner::table4kmbyowner(QObject* parent)
...
QVariant table4kmbyowner::headerData(int section, Qt::Orientation orientation, int role) const
{
...
int table4kmbyowner::rowCount(const QModelIndex& parent) const
{
    if (parent.isValid())
        return 0;
...
int table4kmbyowner::columnCount(const QModelIndex& parent) const
{
    if (parent.isValid())
        return 0;
...
QVariant table4kmbyowner::data(const QModelIndex& index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
bool table4kmbyowner::setData(const QModelIndex& index, const QVariant& value, int role)
{
...
Qt::ItemFlags table4kmbyowner::flags(const QModelIndex& index) const
{
    if (!index.isValid())
...
bool table4kmbyowner::reloadDataFromFile()
{
...
bool table4kmbyowner::saveDataToFile()
{
...
void table4kmbyowner::appendRow(const QList<QVariant>& newRow)
{
...
bool table4kmbyowner::removeRow(int rowIndex)
{
...
QList<QVariant> table4kmbyowner:: getRow(int index)
{
    return _data[index];
}
